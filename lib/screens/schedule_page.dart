...
class _SchedulePageState extends State<SchedulePage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _saveScheduleData() async {
    ...
    // Cross-section conflict: same day selected in both Regular and Other Days with overlapping times
    final crossConflict = _findCrossSectionConflict();
    if (crossConflict != null) {
      showCustomToast(context, "Schedule conflict: $crossConflict", isSuccess: false);
      return;
    }
  ...
  // Returns a human-readable conflict description if any cross-section time overlap
  // exists for the same day, or null if there are no conflicts.
  String? _findCrossSectionConflict() {
    const dayNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

    for (int dayIndex = 0; dayIndex < 7; dayIndex++) {
      // Section A sessions active on this day: [startIdx, endIdx, label]
      final List<List<dynamic>> aRanges = [];
      if (sectionA_morningSessionEnabled && sectionA_morningSelectedDays[dayIndex]) {
        aRanges.add([
          timeOptions.indexOf(sectionA_morningStartTime),
          timeOptions.indexOf(sectionA_morningEndTime),
          'Regular Morning ($sectionA_morningStartTime–$sectionA_morningEndTime)',
        ]);
      }
      if (sectionA_eveningSessionEnabled && sectionA_eveningSelectedDays[dayIndex]) {
        aRanges.add([
          timeOptions.indexOf(sectionA_eveningStartTime),
          timeOptions.indexOf(sectionA_eveningEndTime),
          'Regular Evening ($sectionA_eveningStartTime–$sectionA_eveningEndTime)',
        ]);
      }

      // Section B sessions active on this day
      final List<List<dynamic>> bRanges = [];
      if (sectionB_morningSessionEnabled && sectionB_morningSelectedDays[dayIndex]) {
        bRanges.add([
          timeOptions.indexOf(sectionB_morningStartTime),
          timeOptions.indexOf(sectionB_morningEndTime),
          'Other Morning ($sectionB_morningStartTime–$sectionB_morningEndTime)',
        ]);
      }
      if (sectionB_eveningSessionEnabled && sectionB_eveningSelectedDays[dayIndex]) {
        bRanges.add([
          timeOptions.indexOf(sectionB_eveningStartTime),
          timeOptions.indexOf(sectionB_eveningEndTime),
          'Other Evening ($sectionB_eveningStartTime–$sectionB_eveningEndTime)',
        ]);
      }

      // Check every A-session vs every B-session for overlap on this day.
      // Two ranges [s1,e1) and [s2,e2) overlap when s1 < e2 AND s2 < e1.
      for (final a in aRanges) {
        for (final b in bRanges) {
          final aStart = a[0] as int;
          final aEnd   = a[1] as int;
          final bStart = b[0] as int;
          final bEnd   = b[1] as int;
          if (aStart < bEnd && bStart < aEnd) {
            return '${dayNames[dayIndex]}: ${a[2]} conflicts with ${b[2]}';
          }
        }
      }
    }
